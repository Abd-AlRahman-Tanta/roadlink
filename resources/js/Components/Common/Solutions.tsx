...
const Solutions = ({ title, solutions, button }: { title: string, solutions: string[], button: { text: string, link: string } }) => {
  return (
    <div className=" w-full ">
      <SectionTitle className=" text-buttonBg sectionTitle font-bold mb-9  " children={title} />
      ...
      <Link className="  w-11 h-12 bg-buttonBg text-white rounded-lg flex justify-center items-center " href={button.link} > {button.text} </Link>
    </div>
  )
}
